            ...
            <tbody>
              {postList.map((post, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{post.type}</td>
                  <td>{post.content}</td>
                  <td>{post.pinned ? 'true' : 'false'}</td>
                  <td>{post.eventTime ? <TextFormat type="date" value={post.eventTime} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{post.blogId ? <Link to={`blog/${post.blogId}`}>{post.blogId}</Link> : ''}</td>
