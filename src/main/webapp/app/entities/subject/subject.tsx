...
export const Subject = (props: ISubjectProps) => {
  ...
  const { subjectList, match, loading, totalItems } = props;
  return (
    <div>
            ...
            <tbody>
              {subjectList.map((subject, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{subject.name}</td>
                  <td>{subject.postId ? <Link to={`post/${subject.postId}`}>{subject.postId}</Link> : ''}</td>
                  <td>{subject.blogId ? <Link to={`blog/${subject.blogId}`}>{subject.blogId}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${subject.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${subject.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
      ...
      {props.totalItems ? (
        <div className={subjectList && subjectList.length > 0 ? '' : 'd-none'}>
