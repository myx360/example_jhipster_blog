...
export const Blog = (props: IBlogProps) => {
  ...
  const { blogList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="blog-heading">
        Blogs
      ...
      <div className="table-responsive">
        {blogList && blogList.length > 0 ? (
            ...
            <tbody>
              {blogList.map((blog, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{blog.name}</td>
                  <td>{blog.userId ? <Link to={`site-user/${blog.userId}`}>{blog.userId}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${blog.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${blog.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
          ...
          !loading && <div className="alert alert-warning">No Blogs found</div>
        )}
      </div>
      {props.totalItems ? (
