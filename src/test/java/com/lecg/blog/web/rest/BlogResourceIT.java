...
import com.lecg.blog.repository.BlogRepository;
import com.lecg.blog.service.BlogService;
import com.lecg.blog.service.dto.BlogDTO;
import com.lecg.blog.service.mapper.BlogMapper;
import com.lecg.blog.service.dto.BlogCriteria;
import com.lecg.blog.service.BlogQueryService;
...
public class BlogResourceIT {
    ...
    public void getAllBlogsByUserIsEqualToSomething() throws Exception {
        // Initialize the database
        blogRepository.saveAndFlush(blog);
        SiteUser user = SiteUserResourceIT.createEntity(em);
        em.persist(user);
        em.flush();
        blog.setUser(user);
        blogRepository.saveAndFlush(blog);
    ...
    public void updateBlog() throws Exception {
        ...
        restBlogMockMvc.perform(put("/api/blogs").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content(TestUtil.convertObjectToJsonBytes(blogDTO)))
            .andExpect(status().isOk());
    ...
    public void deleteBlog() throws Exception {
        ...
        // Delete the blog
        restBlogMockMvc.perform(delete("/api/blogs/{id}", blog.getId()).with(csrf())
